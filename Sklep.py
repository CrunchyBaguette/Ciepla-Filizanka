...
def updateCartQuantity(user):
    cart = db["orders"].find_one({"clientId": ObjectId(user), "status": "koszyk"})["itemList"]
    newCart = []
    for c in cart:
        product = db["items"].find_one({"_id": ObjectId(c["itemId"])})
        if c["itemCount"] > product["quantity"]:
            c["itemCount"] = product["quantity"]
        if c["itemCount"] != 0:
            newCart.append(c)
    db["orders"].update_one({"clientId": ObjectId(user), "status": "koszyk"},{
        "$set":{
            "itemList": newCart
        }
    })
